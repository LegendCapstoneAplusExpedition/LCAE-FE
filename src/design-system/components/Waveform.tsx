...
export function Waveform({compact = false}: Props): React.JSX.Element {
  ...
  return (
    <View
      ...
      {values.map((value, index) => (
        <View
          key={index}
          className={`w-[5px] rounded-full ${index < 10 ? 'bg-ink' : 'bg-ink/25'}`}
          style={{height: minHeight + value * (maxHeight - minHeight)}}
        />
      ))}
    </View>
  );
